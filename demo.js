      ...
      if (
        ...
        if (
          ...
          if (!eCompleto) {
            ...
            descargarDatosEntrenamiento(datosEntrenamiento);
            enRedNeural();
            eCompleto = true;
          }
          modoAuto = true;
...
function update() {
    ...
    if (modoAuto == true) {
        ...
        console.log("Salida salto: " + bot[0]);
        console.log("Salida adelante: "  + bot[1]);
        
        if (bot[0] > 0.65 && jugador.body.onFloor()) {
          saltar();
        }
        if (Math.abs(bot[1]) > 0.12 && jugador.x<650) {
          jugador.body.velocity.x = 200;
        } 
      }
...
function disparo() {
    velocidadBalaX = -1 * velocidadRandom(300, 500);
    velocidadBalaY = -1 * velocidadRandom(300, 500);
...
// Función para guardar los datos de entrenamiento en un archivo
function descargarDatosEntrenamiento(datos) {
    const contenidoArchivo = conversionTexto(datos);
    ...
    enlaceDescarga.download = 'datos_entrenamiento.txt';
    enlaceDescarga.click();
}

function conversionTexto(datos) {
  let texto = '';

  for (let i = 0; i < datos.length; i++) {
      const input = datos[i].input.join(' ');
      const output = datos[i].output.join(' ');
